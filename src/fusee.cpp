...
fusee::fusee(int Gen, int BrainSize):Brain(BrainSize), gen(Gen) { // constructeur de la fusée
	// les modules ne sont pas ceux donné dans l'énoncé nous leur avons donné plus de carburant et plus de puissance
	...
	for(int i = 3;i<7;i++){ // on allume les booster, ils ne seront plus jamais éteint
...
void fusee::nextStep(long double t){ // définition de l'évoluition de la fusée
	if(vivant){
		...
		int step = t/15; // on ne veut que les modification du cerveau n'ai lieu que toute les 15sec
		...
		//cout <<test<<endl;
		if (test){ // on vérifie qu'il reste des instruction à éxécuter
		...
		/*if((sqrt(pow(ENIAC.position.first,2)+pow(ENIAC.position.second,2))>6571000) & (sqrt(pow(ENIAC.position.first,2)+pow(ENIAC.position.second,2))<6971000)){
			fit++; // donne uné récompense pour être dans la bonne couche d'orbite
		}*/
	}
